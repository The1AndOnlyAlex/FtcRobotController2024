...
public final class TimeInterpolatableBuffer<T> {
  ...
  public Optional<T> getSample(double timeSeconds) {
    ...
    if (topBound == null && bottomBound == null) {
      ...
      return Optional.of((T)bottomBound.getValue());
    } else if (bottomBound == null) {
      return Optional.of((T)topBound.getValue());
    } else {
      ...
      return Optional.of(
          m_interpolatingFunc.interpolate(
                  (T)bottomBound.getValue(),
                  (T)topBound.getValue(),
              (timeSeconds - (double)bottomBound.getKey()) / ((double)topBound.getKey() - (double)bottomBound.getKey())));
    }
  }
